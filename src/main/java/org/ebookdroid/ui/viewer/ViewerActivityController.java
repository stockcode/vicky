...
public class ViewerActivityController extends AbstractActivityController<ViewerActivity> implements
    ...
    public void afterCreate(final ViewerActivity activity, final boolean recreated) {
        ...
        audioFile = FileUtils.getAudioFile(m_fileName);
    ...
    private void setAudioUri() {
        if (audioFile != null) {
                ...
                player = MediaPlayer.create(getManagedComponent(), Uri.fromFile(audioFile));
    ...
    public void onDestroy(final boolean finishing) {
        ...
        if (audioFile.exists() && player.isPlaying()) {
            player.stop();
            player.release();
        }
    }
        ...
        final Runnable r = new Runnable() {
            ...
            public void run() {
                ...
                if (pageCount > 0) {
                    ...
                    audioFile = FileUtils.getAudioFile(m_fileName, newIndex.viewIndex + 1);
                    if (audioFile.exists()) {
                        setAudioUri();
                        player.start();
                    }
    ...
    @ActionMethod(ids = R.id.mainmenu_playaudio)
    public void playAudio(final ActionEx action) {
